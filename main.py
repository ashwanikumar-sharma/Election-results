...
def scrape_election_data(url):
    ...
    for candidate in candidates:
        ...
        status_div = cand_info.find('div', class_='status leading')
        if status_div:
            ...
            if status.lower() == 'leading':
                votes = status_div.find('div').text.strip() if status_div.find('div') else "No Votes"
                vote_margin = status_div.find('span').text.strip('()') if status_div.find('span') else "No Vote Margin"
